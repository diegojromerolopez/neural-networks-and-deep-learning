...
def plot_errors(error_locations, erroneous_predictions=None):
    ...
    for j in range(n):
        ...
        ax.matshow(error_images[j], cmap=plt.cm.get_cmap("binary"))
...
def plot_filters(net, layer, x, y):
    ...
    for j in range(len(filters)):
        ...
        ax.matshow(filters[j][0], cmap=plt.cm.get_cmap("binary"))
        plt.xticks(np.array([]))
        plt.yticks(np.array([]))
    plt.tight_layout()
    return plt
...
def run_experiments():
    ...
    plot = plot_errors(error_locations, erroneous_predictions)
    plot.savefig("ensemble_errors.png")
    # plot the filters learned by the first of the nets just trained
    plot = plot_filters(nets[0], 0, 5, 4)
    plot.savefig("net_full_layer_0.png")
    plot = plot_filters(nets[0], 1, 8, 5)
    plot.savefig("net_full_layer_1.png")
